...
function wpc_latest_product_shortcode( $atts ) {
	...
	wp_reset_query();
	$prefix_string = '<div class="heading-wooCommerce-product-carousel"><h3>Latest Products</h3></div>';
	if(empty($thePosts)){
...
function wpc_featured_shortcode( $atts ) {
	...
	wp_reset_query();
	$prefix_string = '<div class="heading-wooCommerce-product-carousel"><h3>Featured Products</h3></div>';
	if(empty($thePosts)){
	    $return_string= '<div class="no-data"><strong>No Product Yet.</strong></div>';
	}
	return $prefix_string.$return_string;
